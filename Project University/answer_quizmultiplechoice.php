...
<html>
   ...
   <body>
       ...
       <h3>INSTRUCTION : PLEASE ANSWERING ALL THIS QUESTION.</h3>
       <br>
...
$sqledit = "INSERT INTO student_multiplechoice_marks (student_ID,student_name,course_name,marks) values ('$newid','$newname','$getid','0')";
...
$sql = "SELECT question FROM quizmultiplechoice where course_name = '$getid'";
$var = 0;
$var2 = 0;
...
echo "<form action='quizmultiplechoice_done.php?getid=$var2&getcourse=$getid' method='POST'>";
  while($row = $result->fetch_assoc()) {
     $id = $row['question'];  
     $var = $var +1;
     $name = 'answer_' . $var;
      
    $html = '<tr><td>'.$var.'</td><td>'.$row['question'] . '</td><td><input type="radio" name="' . $name . '" value="A" /></td><td> <input type="radio" name="' . $name . '" value="B" > </td><td> <input type="radio" name="' . $name . '" value="C" > </td><td> <input type="radio" name="' . $name . '" value="D" ></tr></td> <br/>';
